...
(defn reducer-1
  [[start count] command]
  (let [end (rotate start command)] [end (if (= end 0) (inc count) count)]))

(defn solve-1
  [input]
  (let [commands (map parse-command input)
        start 50]
    (get (reduce reducer-1 [start 0] commands) 1)))
...
(defn rotate-count-0
  ...
  (case dir
    "R" (let [pseudo-end (+ start len)
              count (inc (int (- (floor (/ pseudo-end 100))
                                 (ceil (/ (inc start) 100)))))
              end (mod pseudo-end 100)]
          [end count])
    "L" (let [pseudo-end (- start len)
              count (inc (int (- (floor (/ (dec start) 100))
                                 (ceil (/ pseudo-end 100)))))
              end (mod pseudo-end 100)]
          [end count])))
...
(defn reducer-2
  [[start count] command]
  (let [[end add-count] (rotate-count-0 start command)]
    [end (+ count add-count)]))
